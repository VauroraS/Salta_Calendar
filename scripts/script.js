document.addEventListener("DOMContentLoaded", function () {
    ...
    async function renderCalendar() {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const eventsForDay = await checkEventsForDay(day);
    ...
    async function checkEventsForDay(day) {
        try {
            const response = await fetch("eventos.json");
            const data = await response.json();

            const eventsForDay = data.eventos.filter(event => {
    ...
    async function showEventCard(day) {
        ...
        try {
            const data = await fetch("eventos.json").then(response => response.json());
